...
| `access_token` | VARCHAR(2048) | ✅ | **LINA 발급** access token(세션, RS256 JWT). Confluence 토큰 아님(§6.2). 512→2048 확장(2026-06-11, RS256 길이). admin 사전 seed 는 로그인 전이라 `'ADMIN_PLACEHOLDER'` 더미값으로 INSERT, 첫 로그인 시 진짜 JWT 로 덮어씀(§6.1 admin seed) |
...
Confluence OAuth access/refresh token + `cloud_id` 저장. **앱 내 Confluence 페이지 미리보기**(라이브 REST 호출) 및 **ingestion 콘텐츠 조회**(OAuth Bearer + 게이트웨이 URL `/ex/confluence/{cloudId}/...`)에 사용한다 — 챗(`/ml/query`)은 토큰 불필요. 사용자당 1:1. (admin-key 관리용 정적 API Token·site URL 은 본 테이블이 아니라 §6.4 `admin_atlassian_credential` 에 별도 보관.)
...
| `site_url` | VARCHAR(255) | ✅ | Confluence base URL `https://{site}.atlassian.net`. 용도 ① admin-key 관리 API `POST/DELETE {site_url}/wiki/api/v2/admin-key`(콘텐츠 조회 게이트웨이와 별개), ② §2-5 응답에 **`siteUrl`**(JSON, camelCase)로 ingestion 에 반환(출처 URL absolute 정규화용). 별도 저장 없이 본 컬럼 1곳에서 양쪽 사용. **DB 컬럼=`site_url`, JSON 필드=`siteUrl`** |
...
> **등록 경로 확정(2026-06-12) — 로그인 전 사전 seed**: admin 정보는 로그인 전에 미리 채운다(§6.1 admin seed 와 한 묶음).
> - **`site_url`** — Flyway 마이그레이션으로 평문 seed(`users` admin 행과 함께).
> - **`admin_api_token_enc`** — 평문 SQL 불가(AES-GCM 컬럼, 암호문 하드코딩 시 secret 이 git 에 남음). **앱 기동 seeder**(`ApplicationRunner`)가 env(k8s Secret) 의 평문 API Token 을 `TokenCipher` 로 암호화 후 행이 없을 때 INSERT → git 에 secret 미포함. adminEmail=`users.email`.
> - 본 테이블은 **OAuth 무관**이라 사전 seed 만으로 완비된다 → Feature 6(admin-key activate/deactivate)은 로그인 없이 동작. (단 Feature 5 credential 조회는 `user_tokens` 가 로그인 산물이라 admin 1회 로그인 필요 — §6.2)

...
- **스페이스 식별자 · `feature13` · 변경 이력 (C·D·E)**: (C) `spaceKey`(고유 키·스코프) / `spaceId`(내부 ID·출처) / `spaceName`(표시 라벨) 3종 구분을 Common 에 명시(주: 2단계 단일 고정 스페이스에선 사실상 무의미, 멀티 스페이스 3단계+에서 유효 — 실제 단순화 후보는 `spaceName` 이 아니라 `spaceKey`↔`spaceId`). (D) 미정의 마커 `feature13` → 서술형("ML 응답을 BE 통합 목표 계약으로 마이그레이션 이후")으로 교체. (E) `## 변경 이력`(v2.2.0) 신설 + "문서 자체 버전 ≠ 기획서 버전" 명시. 변경 이력 위치를 **헤더 직후 상단**으로 옮기고 `docs/db-schema.md` 도 동일하게 통일(§6 → 상단 무번호 섹션).

...
- **설정 시 (SPA 핸드오프)** — 브라우저에 JSON 을 노출하지 않고 FE 콜백 라우트로 **302** 한다. 결과는 `Location` 쿼리로 전달한다:
  ...
  `returnTo` 는 **로그인 `mode`** 로 결정한다(role 이 아님): 관리자 로그인(`mode=admin`)→`/admin`, 사용자 로그인(`mode` 생략·기타값)→`/chat`. `mode=admin` 은 위 admin 게이트가 `users.role == ADMIN` 을 이미 보장하므로 `/admin` 진입이 안전하고, **ADMIN 계정이라도 사용자 로그인(`mode` 생략)으로 들어오면 `/chat`** 으로 보낸다(관리자가 일반 채팅을 이용하려는 의도 존중). `mode` 는 `state` 에 서버 보관되어 클라이언트가 위조할 수 없다. FE `AuthCallbackPage` 는 `accessToken` 을 보관(localStorage)한 뒤 `returnTo` 로 라우팅하고, `errorCode` 가 있으면 `/login?error={FORBIDDEN|AUTH_FAILED}` 로 이동한다. 노출되는 토큰은 **LINA 세션 JWT** 뿐이다.

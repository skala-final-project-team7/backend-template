...
>
> **Admin Key 말소 (보안)**: ingestion job 완료 직후 **ML(Data Ingestion Pipeline) 측이 Atlassian admin-key deactivate API 를 호출**해 키를 즉시 폐기한다. 60분 TTL 자동 만료는 fallback. BE 는 deactivate 호출 책임을 지지 않는다 — ML 팀이 ingestion worker 종료 단계에 포함.
...
[프론트엔드]
  ...
  ├─ POST   /api/admin/ingest                        → BFF: key 활성 미확인 시 자동 activate → POST /ml/ingest
  │                                                    └─ ML: ingestion 완료 후 admin-key deactivate 호출(보안)

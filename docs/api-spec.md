...
- `spaceKey` (예 `"CPC"`): 사람이 정한 **짧은 고유 키**(불변). 스코프 지정용. 사용처: `/ml/query` (**선택** — 생략 시 사용자 접근 가능 모든 스페이스 cross-space 검색), `/ml/ingest` (**필수** — admin 이 수집 대상 명시), `/api/admin/ingest` (**필수**).
...
> 두 가지 모드: (a) `spaceKey` 생략/빈 값 → **cross-space** 검색 (기본 UX, 사용자가 접근 가능한 모든 스페이스에서 답변). (b) `spaceKey` 지정 → 해당 스페이스로 한정. 어느 쪽이든 `userId`/`groups` ACL 은 항상 적용된다.
...
| `spaceKey`       | String   | —        | **선택**. 검색 대상 스페이스. 생략(또는 `null`/빈 문자열) 시 사용자가 접근 가능한 **모든 스페이스** cross-space 검색(`groups`/`userId` ACL 만 적용). 특정 스페이스로 좁히고 싶을 때만 지정.       |
...
> **ACL fail-closed (BFF 측 강제)**: `userId` 가 비어 있거나 `groups` 가 비어 있으면(`[]`) BFF 는 `/ml/query` 호출을 **막고** SSE `error`(`errorCode: UNAUTHORIZED`)로 종료한다 (`backend/CLAUDE.md` §6 "ACL 필터 없이 RAG 호출 금지"). RAG 자체 검증이 빈 값을 허용하더라도 BFF 가 게이트한다. **`spaceKey` 는 ACL 이 아니라 스코프 필드이므로 게이트 대상이 아니다** — 누락은 cross-space 검색으로 해석한다(2026-06-02 결정).

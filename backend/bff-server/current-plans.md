...
- [x] ML 호출 시 다음을 전달 (`docs/api-spec.md` §2-1):
  ...
  - **`/ml/query` 본문에 `accessToken`/`cloudId` 미포함** (확정된 결정 #5, 2026-05-22 갱신). 2026-06-05 갱신: `/ml/ingest`/RabbitMQ job payload 에도 Confluence credential set 을 포함하지 않는다. 2026-06-11 정정: admin-key 경로는 OAuth accessToken 이 아니라 auth-server 내부 API 에서 조회한 admin API Token + adminEmail + cloudId 를 사용한다.
...
- [x] **PoC 토큰 보안 (Data Ingestion 호출 경로 한정)**: admin API Token 을 로그·tracing 본문에 노출하지 않음 (마스킹), HTTP/RabbitMQ job·event payload 에 `adminApiToken`/`adminEmail`/`accessToken`/`refreshToken`/`cloudId` 미포함, actuator 민감 endpoint 비노출 — `docs/api-spec.md` §2-2 보안 주의 준수 (RAG 질의 경로엔 토큰 자체가 없으므로 본 규칙 적용 대상은 ingestion 호출)
...
5. **admin-ingest credential 조회 모드** (2026-05-21, 2026-05-22, 2026-06-05 갱신, 2026-06-11 정정): **credential payload 미포함 모드로 정정** — BFF 는 Data Ingestion Pipeline 요청 본문 또는 RabbitMQ payload 에 `adminApiToken`/`adminEmail`/`accessToken`/`refreshToken`/`cloudId` 를 첨부하지 않는다. Data Ingestion Worker 가 `adminUserId` 로 auth-server 내부 credential 조회 API 를 호출해 admin API Token + adminEmail + cloudId 를 함께 조회한다(`docs/api-spec.md` §2-2). admin-key 활성화/적용 read 경로는 OAuth Bearer accessToken 이 아니라 `Authorization: Basic base64(adminEmail:adminApiToken)` + `Atl-Confluence-With-Admin-Key: true` 를 site URL 에서 사용한다. 운영 보호장치(로그·tracing 본문 마스킹, actuator 민감 endpoint 차단, NetworkPolicy, RabbitMQ payload credential 미포함) 동반 필수. 관련 설정 키: `lina.data-ingestion.*` (Data Ingestion 호출 클라이언트 base-url/timeout). 상세는 `backend/auth-server/current-plans.md` §3단계 Feature A~D 참조.
   - **2026-05-22 갱신 / 2026-06-05 정정 / 2026-06-11 정정**: credential 사용 대상을 RAG 질의(`/ml/query`, §2-1) → 데이터 수집(`/ml/ingest`, §2-2) 로 변경하되, 전달 방식은 payload 첨부가 아니라 Data Ingestion Worker 의 auth-server 내부 credential 조회로 정정. 2026-06-11 정정으로 admin-key 경로의 credential 은 OAuth accessToken 이 아니라 admin API Token + adminEmail + cloudId 로 확정한다. **근거**: 권한은 수집 시 Qdrant payload(`allowed_groups`/`allowed_users`) 에 ACL 저장 + 질의 시 JWT 의 `userId`/`groups` 로 필터링 (기획서 §6.4/§6.6). 따라서 `/ml/query` 는 라이브 Confluence 호출이 없어 Confluence credential 불필요, admin API Token 은 크롤하는 수집 단계에서만 내부 조회해 사용. **전제**: "`/ml/query` 가 실시간 Confluence 호출을 일절 안 함" 을 가정 (※ ML 확인 대기 — 확인 후 본 결정 확정).
...
| `POST /api/admin/ingest` | `jobId`/`status`(`STARTED`)/`startedAt` | RabbitMQ ingest job 또는 ML 서버 `POST /ml/ingest` | body `{ mode }` (생략 시 `"full"`). **스페이스 스코프 없음** — admin Key 로 admin 이 접근 가능한 모든 스페이스 일괄 크롤(2026-06-04 결정). **내부 처리(2026-06-05 갱신 — watcher polling 대체, 2026-06-11 credential 모델 정정)**: ① BFF 가 admin 의 Admin Key 활성 상태 확인 → 만료/미활성이면 auth-server 통해 자동 activate (회의 결정 2026-06-02) ② BFF 가 `jobId` 생성 후 RabbitMQ ingest job 을 직접 발행하거나 Data Ingestion Pipeline 의 `/ml/ingest` 를 호출해 job 발행 위임 ③ job payload 는 `jobId`/`adminUserId`/`mode` 등 식별·상태 정보만 포함, `adminApiToken`/`adminEmail`/`accessToken`/`refreshToken`/`cloudId` 미포함 ④ Data Ingestion Worker 가 `adminUserId` 로 auth-server 내부 credential 조회 API 에서 admin API Token + adminEmail + cloudId 를 함께 조회 ⑤ Data Ingestion 이 site URL 의 Atlassian REST 호출 시 `Authorization: Basic base64(adminEmail:adminApiToken)` + `Atl-Confluence-With-Admin-Key: true` 헤더 부여 ⑥ Data Ingestion Pipeline 이 RabbitMQ completion event 발행 ⑦ BFF consumer 가 completion event 를 consume하고 auth-server `POST /internal/admin/key/deactivate` 호출 → Atlassian Admin Key 폐기. 2단계 demo 데이터 셋업도 본 endpoint 사용. (`docs/api-spec.md` §1-4·§2-2) |
...
공통: `/api/admin/*` ADMIN 전용(미인증 401·일반 403). PoC 토큰 보안(평문 로그 노출 금지·HTTP/RabbitMQ payload credential 미포함·NetworkPolicy) — `docs/api-spec.md` §2-2 보안 주의 참조. **검증 게이트**: Basic auth(adminEmail:adminApiToken) + Admin Key 헤더 동작은 admin API Token 확보 직후 site URL 기준 curl 로 확인(`backend/auth-server/current-plans.md` Feature A 게이트 항목).
...
Credential 처리 원칙:
- 기존 2026-06-04 흐름의 `/ml/ingest` 본문 직접 전달(`accessToken` + `cloudId`)은 사용하지 않는다. 2026-06-11 정정으로 admin-key 경로는 OAuth accessToken 이 아니라 admin API Token + adminEmail + cloudId 를 사용한다. watcher polling 은 원래 credential 전달 수단이 아니라 완료 감지/deactivate 트리거였으며, 이 책임만 completion event 로 대체한다.
- BFF 는 `/api/admin/ingest` 처리 중 `adminApiToken`/`adminEmail`/`accessToken`/`refreshToken`/`cloudId` 를 조회하거나 Data Ingestion Pipeline/RabbitMQ payload 에 전달하지 않는다.
- BFF 또는 Data Ingestion Pipeline 이 발행하는 ingest job payload 는 `jobId`, `adminUserId`, `mode`, `requestedAt` 등 식별 정보만 포함한다.
- Data Ingestion Worker 는 job consume 후 `adminUserId` 로 auth-server 내부 credential 조회 API 를 호출해 admin API Token + adminEmail + cloudId 를 함께 받는다.
- Data Ingestion Worker 는 admin-key 가 적용되는 Confluence 호출을 site URL 에서 수행하며 `Authorization: Basic base64(adminEmail:adminApiToken)` + `Atl-Confluence-With-Admin-Key: true` 헤더를 사용한다. OAuth accessToken/Bearer 는 admin-key 경로에 사용하지 않는다.

- [ ] BFF ingest 요청 처리에서 `jobId` 생성, Admin Key activate, RabbitMQ ingest job 발행 또는 Data Ingestion `/ml/ingest` 호출 위임 경계를 확정
- [ ] ingest job payload schema 정의: `jobId`, `adminUserId`, `mode`, `requestedAt` 중심. `adminApiToken`/`adminEmail`/`accessToken`/`refreshToken`/`cloudId` 포함 금지
- [ ] Data Ingestion Worker 가 `adminUserId` 로 auth-server 내부 credential 조회 API 에서 admin API Token + adminEmail + cloudId 를 함께 조회하는 계약 확인

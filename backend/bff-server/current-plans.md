...
- Data Ingestion Worker 는 job consume 후 `adminUserId` 로 auth-server 내부 credential 조회 API 를 호출해 **admin OAuth accessToken + cloudId** 를 함께 받는다(콘텐츠 조회용).
...
- [ ] Data Ingestion Worker 가 `adminUserId` 로 auth-server 내부 credential 조회 API 에서 admin OAuth accessToken + cloudId 를 함께 조회하는 계약 확인

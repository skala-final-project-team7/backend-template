...
`andrej-karpathy-skills/CLAUDE.md` 는 구현/리뷰/리팩터링 시 보조 개발 원칙으로 함께 적용한다.
단, 루트 `CLAUDE.md`, `backend/CLAUDE.md`, `docs/...`, `backend/rules/...`, 본 문서의 구체 규칙이 항상 우선한다.
...
### 2.1 보조 개발 원칙
- 구현 전 불명확한 가정, 해석 차이, 영향 범위를 먼저 드러낸다.
- 요청 범위를 넘는 기능, 추상화, 리팩터링을 추가하지 않는다.
- 변경 라인은 작업 목표와 직접 연결되어야 하며, 기존 스타일을 우선 따른다.
- 각 작업은 완료 기준과 검증 방법을 정하고, 테스트 또는 명령 실행 결과로 확인한다.

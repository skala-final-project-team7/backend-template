...
- Backend는 MongoDB에 조회만 수행한다. 쓰기는 Ingestion/Sync Worker가 담당한다.
- OAuth 토큰은 MySQL에 암호화 저장한다. 평문 저장 금지.
...
- MongoDB에 쓰기 작업을 수행하지 않는다 (Backend는 읽기 전용).

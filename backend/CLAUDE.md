...
- OAuth 토큰은 MySQL에 암호화 저장한다(3단계). 평문 저장 금지. (2단계에서는 MySQL 미사용 — `users`(`role` 컬럼 포함)/`user_groups`/`user_tokens`는 3단계에서 도입. 별도 `admins` 테이블은 `users.role` 컬럼으로 흡수 — 2026-06-02 결정, `docs/db-schema.md` §6. `groups`는 로그인 시 `memberof`로 조회해 `user_groups` 테이블에 적재→JWT claim(`groupId`). 스페이스 단위 권한 테이블 `user_space_acl`은 미사용.)

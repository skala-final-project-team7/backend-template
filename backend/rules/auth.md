...
- JWT에 포함할 Claim: `userId`(Confluence accountId), `groups`(사용자 Confluence group 멤버십 = **`groupId` 배열**, 인증 시 `memberof` API 로 취득 — 질의 시 RAG ACL 필터, `docs/adr/0001-page-level-acl-source.md` §2), `role`(`USER`/`ADMIN` — MySQL `users.role` 단일 source, `docs/db-schema.md` §6.1). Claim 이름은 와이어 필드(`/ml/query`)·JSON 표면과 동일하게 **camelCase** 로 통일한다 (`docs/api-spec.md` §2-1, `docs/adr/0001-page-level-acl-source.md` §2.3).

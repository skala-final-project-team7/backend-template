...
| 3단계 | Confluence OAuth 2.0 Authorization Code Flow + Access/Refresh Token 암호화 저장 + JWT 발급 | 🚧 진행 — Feature 0(게이트)·1(영속)·2(JWT)·3(login/callback)·4(세션)·5(credential 조회)·**6(admin-key activate/deactivate)** 코드 **구현 완료**(F6=내부 API + WireMock 단위까지, 외부 연동 통합은 4단계). **Feature 7**(보안 운영)·8(검증) 잔여. 실 연동 잔여: `X-Internal-Api-Key`↔Ingestion 합의·admin seed 구현(설계 확정 — 로그인 전 사전 seed)·users/me(BFF). (게이트 종료 2026-06-10 하이브리드, 착수 전 결정 6건 전부 확정) |
